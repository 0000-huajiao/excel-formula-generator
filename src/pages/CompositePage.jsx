...
// ─── TipInput：带气泡提示的输入框（module-level，稳定引用）─

function TipInput({ tip, wrapClass = '', ...inputProps }) {
  ...
  return (
    ...
    >
      {show && tip && (
        <div className="absolute bottom-full left-2 mb-1.5 z-40 pointer-events-none">
          <div className="bg-gray-900 dark:bg-gray-700 text-white text-xs px-2.5 py-1.5 rounded-lg shadow-lg whitespace-nowrap max-w-[240px] leading-snug">
            {tip}
          </div>
          <div className="ml-2 w-0 h-0" style={{ borderLeft: '5px solid transparent', borderRight: '5px solid transparent', borderTop: '5px solid #111827' }} />
        </div>
      )}
      <input {...inputProps} />
    </div>
...
// ─── BubbleWrapper：类型确认气泡（module-level，稳定引用）──

function BubbleWrapper({ condId, field, bubble, bubbleRef, onSelect, bubbleValue, children }) {
  ...
  return (
    <div className="relative flex-1 min-w-0">
      ...
      {show && (
        <div ref={bubbleRef}>
          <TypeBubble value={bubbleValue || ''} onSelect={onSelect} />
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="mt-3 default-row flex flex-wrap items-center gap-2">
      ...
      <TipInput
        ...
        className="param-input text-sm w-full"
        value={value}
...
function AggRow({ idx, c, onUpdate, onRemove, canRemove }) {
  return (
    <div className="condition-row flex flex-wrap items-center gap-2">
      ...
      <TipInput
        className="param-input text-sm w-28 min-w-0"
        value={c.criteriaRange}
      ...
      <TipInput
        wrapClass="flex-1 min-w-0"
        className="param-input text-sm w-full min-w-0"
        value={c.criteriaValue}
        onChange={e => onUpdate({ criteriaValue: e.target.value })}
        placeholder='如 "销售部" 或 ">1000"'
        tip='条件值，如 "销售部"、">1000"、A2'
        aria-label="条件值"
      />
...
export default function CompositePage({ onBack }) {
  ...
  const [bubble, setBubble] = useState(null) // { condId, field }
  ...
  const bp = { bubble, bubbleRef, onSelect: handleTypeSelect }
          ...
          {conditions.map((c, idx) => (
            <div key={c.id} className="condition-row space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <BubbleWrapper condId={c.id} field="right" {...bp} bubbleValue={c.right}>
                  <TipInput
                    wrapClass="w-full"
                    className={`${miniInput} w-full`}
                    value={c.right}
                    ...
                    placeholder="阈值，如 10000 或 &quot;完成&quot;"
                    tip='阈值，如 10000、"完成"、B2'
                    aria-label="阈值"
                  />
                </BubbleWrapper>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <BubbleWrapper condId={c.id} field="result" {...bp} bubbleValue={c.result}>
                  <TipInput
                    wrapClass="w-full"
                    className={`${miniInput} w-full`}
                    value={c.result}
                    ...
                    placeholder='返回值，如 A2*0.2 或 "达标"'
                    tip='返回值，如 A2*0.2、"达标"、C2'
                    aria-label="返回值"
                  />
                </BubbleWrapper>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <BubbleWrapper condId={c.id} field="result" {...bp} bubbleValue={c.result}>
                  <TipInput
                    ...
                    onChange={e => patchCondition(c.id, { result: e.target.value, resultType: null })}
                    onBlur={e => handleBlur(c.id, 'result', e.target.value)}
                    placeholder='返回值，如 "及格" 或 A2*0.1'
                    tip='返回值，如 "及格"、A2*0.1、C2'
                    aria-label="返回值"
                  />
                </BubbleWrapper>
              </div>
          ...
          {conditions.map((c, idx) => (
            <div key={c.id} className="condition-row space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <BubbleWrapper condId={c.id} field="matchValue" {...bp} bubbleValue={c.matchValue}>
                  <TipInput
                    wrapClass="w-full"
                    className={`${miniInput} w-full`}
                    value={c.matchValue}
